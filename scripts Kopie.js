...
(function () {
  ...
  // Default: uses window.prompt. You can replace this later with a nicer modal UI.
  if (!window.__py_input__) {
    window.__py_input__ = async function (promptText = "") {
      const value = window.prompt(String(promptText ?? ""));
      if (value === null) {
        // simulate EOF / cancel
        throw new Error("EOFError: input() aborted");
      }
      return String(value);
    };
  }
...
`;
  ...
  async function __ensureInputPatchedOnMain() {
    ...
    if (!window.__mainPy?.runPythonAsync) return;
    await window.__mainPy.runPythonAsync(__INPUT_PATCH__);
  ...
  function looksLikeTopLevelGameLoop(code) {
    ...
    const hasTopWhile = /^while\s+\w+\s*:\s*$/m.test(t) || /^while\s+True\s*:\s*$/m.test(t);
    return hasPygame && hasTopWhile;
  }
  ...
  function wrapTopLevelIntoAsyncMain(userCode) {
    ...
    if (!bodyText.trim()) return code;

    // Inject yields after tick/flip (we'll indent later), preserving indentation
...
`);
  ...
  async function runUserCode(rawFromEditor) {
    ...
    const needsInput = /\binput\s*\(/.test(__userCodeRaw);
    ...
    const wantMain = hasUserFiles || needsCanvasOrDOM(__userCodeRaw) || needsFilesystem(__userCodeRaw) || needsInput;
    ...
    if (wantMain && (!window.isMainMode || !window.isMainMode())) {
      ...
      // ===== input()-Patch (NUR einmal, direkt vor User-Code) =====
      if (window.isMainMode && window.isMainMode() && !window.__inputPatched) {
        try {
          await window.__mainPy.runPythonAsync(__INPUT_PATCH__);
          window.__inputPatched = true;
        } catch (e) {
          console.warn("[input patch] failed:", e);
        }
      }
