...
`);
  ...
  function __initUIAndBoot() {
    ...
    document.getElementById("run-icon")?.addEventListener("click", async () => {
      ...
      try {
        const btn = document.getElementById("run-icon");
        if (window.bootstrap && btn) bootstrap.Tooltip.getOrCreateInstance(btn).hide();
        btn?.blur?.();
      } catch { }
  ...
  window.addEventListener(
    ...
    (e) => {
      ...
      if (document.activeElement === c) {
        // Canvas hält noch Fokus → zum Editor umleiten
        e.preventDefault();
  ...
  function setupUpload() {
    ...
    input.addEventListener("change", async (e) => {
      try {
        ...
        const sel = document.getElementById("layout-select");
        const prev = getLS("ui.layout", sel?.value || "output-files");
        const canvasShownOrActive = !!window.__canvasActive || /^canvas-/.test(prev);
        const next = canvasShownOrActive ? "canvas-output-files" : "output-files";
  ...
  window.refreshFileList = function () {
    ...
    // Wenn Dateien vorhanden sind und das Layout noch auf "canvas-output" steht,
    // automatisch auf "canvas-output-files" umschalten (Menü + LocalStorage synchron).
    try {
      const sel = document.getElementById("layout-select");
      const cur = sel?.value || getLS("ui.layout", "output-files");
      if (files.length > 0 && cur === "canvas-output") {
  ...
  window.applyLayoutByValue = function applyLayoutByValue(val) {
    ...
    switch (val) {
      case "canvas-output":
        console.log("[Layout] → Grafikmodus");
        applyLayout({
          showCanvas: true,
          showFiles: false,
        ...
        break;
      case "output-files":
        console.log("[Layout] → Text/Dateien");
        applyLayout({
          showCanvas: false,
          showFiles: true,
      ...
      case "canvas-output-files":
        ...
        break;
      default:
        console.log("[Layout] → Default (Grafik)");
        applyLayout({
          showCanvas: true,
          showFiles: false,
          outputFilesClass: false,
  ...
  async function runUserCode(rawFromEditor) {
    ...
    // --- NEU: falls Dateien vorhanden, vor der Ausführung sofort Main-Modus erzwingen ---
    // --- UI/Canvas vorbereiten (wie früher) ---
    healEmscriptenInput?.();                 // << NEU blieb
    const _c = getActiveCanvas?.();
    if (_c) _c.getContext("2d")?.clearRect(0, 0, _c.width, _c.height);
    const outEl = document.getElementById("output");
    outEl.innerHTML = "";
    // Notfall-Reset-Flag beim Start löschen
    try { window.__abortFlag = false; } catch { }
    clearAllCanvases?.();
    ...
    if (needsCanvasOrDOM(__userCodeRaw)) {
      const cur = (window.getCurrentLayout?.() || getLS('ui.layout', 'output-files'));
      if (cur === 'output-files') {
    ...
    const wantMain = hasUserFiles || needsCanvasOrDOM(__userCodeRaw) || needsFilesystem(__userCodeRaw);
    ...
    if (isMainMode()) {
      prelude += [
        ...
        "def __check_abort():",
        "    if getattr(js.window, '__abortFlag', False):",
        "        raise KeyboardInterrupt('Programm durch Notfall-Reset abgebrochen')",
        "",
        "import builtins",
    ...
    function transformSyncPygameToAsync(txt) {
      ...
      const hasEventAPI = (t) =>
        /\bpygame\.event\.(get|poll|wait|peek|pump)\s*\(|\bpygame\.display\.flip\s*\(|\bclock\.tick\s*\(/.test(t);
      const hasFrameYield = (t) =>
        /\bpygame\.display\.flip\s*\(\s*\)|\bclock\.tick\s*\(/.test(t);
      ...
      // --- Fallback-Pass: falls aus irgendeinem Grund noch kein await vorhanden ist ---
      const needsFallback =
        /\bimport\s+[^#\n]*\bpygame\b/i.test(patched) &&
        /(^|\n)\s*while\b[^\n:]*:\s*(?:#.*)?$/m.test(patched) &&
        !/await\s+asyncio\.sleep\(\s*0\s*\)/.test(patched);

      if (needsFallback) {
        // nach flip()
        patched = patched.replace(
          /(^[ \t]*)(.*pygame\.display\.flip\s*\([^\n]*\)[ \t]*)(?:\n|$)/gm,
          (_m, ind, call) => `${ind}${call}\n${ind}await asyncio.sleep(0)\n`
        );
        // nach clock.tick(...)
        patched = patched.replace(
          /(^[ \t]*)(.*\bclock\.tick\s*\([^\n]*\)[ \t]*)(?:\n|$)/gm,
        ...
        console.warn("[TRANSFORM] Fallback yields injected after flip()/clock.tick().");
      }
    ...
    if (wantsPygame) {
      ...
      prefix += [
        ...
        "        MAXHEIGHT = int(wrap.clientHeight)",
        "    else:",
      ...
      window.applyCanvasAndPygameSize = async function (W, H) {
        ...
        const ch = Math.max(1, Math.floor(wrap?.clientHeight || H || 0));
    ...
    // + __pygameHeaderLines: Zeilen, die transformSyncPygameToAsync VOR den eigentlichen Usercode gesetzt hat
    const totalPrefixOffset = preludeLines + prefixLines + (__pygameHeaderLines || 0);
  ...
  async function bootPyodide() {
    ...
    if (window.USE_PYODIDE_WORKER && window.PY_WORKER_URL && window.PyClient) {
      try {
        ...
        outEl.innerHTML += "✅ Pyodide (Worker) geladen<br>";
        ...
        await window.registerTurtleInPython?.(window.__mainPy);
        outEl.innerHTML += "💾 Filesystem (Main) bereit<br>";
...
`);
  ...
  document.getElementById("clear-canvas-btn")?.addEventListener("click", () => {
    const appCanvas = getActiveCanvas();
    if (appCanvas)
      appCanvas
        .getContext("2d")
        .clearRect(0, 0, appCanvas.width, appCanvas.height);
  });
  ...
  async function loadExamplesFromJson(jsonUrl = "data/examples.json") {
    ...
    if (!out.length) {
      out.push({
        name:
          dev === "c12"
            ? "Beispiel (Calliope 1/2 – Fallback)"
            : dev === "c3"
              ? "Beispiel (Calliope 3 – Fallback)"
              : "Beispiel (Turtle – Fallback)",
        file: "",
